...
const askQuestion = async (incomingMessageDiv, userMessage) => {
  ...
  try {
    ...
    // Send the query and response to the backend to save them
    saveQueryToServer(userMessage, aiResponse);
...
// Function to send query and response to the server
const saveQueryToServer = async (query, response) => {
  try {
    const saveEndpoint = "http://localhost:3000/save-query"; // Adjust to match your backend endpoint
    ...
    console.error("Error saving query and response:", error);
  }
};
